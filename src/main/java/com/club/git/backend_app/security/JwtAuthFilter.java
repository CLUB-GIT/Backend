...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // Authentifier seulement si pas déjà authentifié dans le contexte
        if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            Membre membre = membreRepository.findByEmail(email).orElse(null);

            if (membre != null && jwtService.isTokenValid(jwt, membre)) {
                ...
                log.debug("Membre authentifié : {} [{}]", email, membre.getRole());
            }
        }
